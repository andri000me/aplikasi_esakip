...
class IndikatorkegiatanController extends Controller
{
	...
	public function accessRules()
	{
		return array(
            ...
            array('allow', // allow admin user to perform 'admin' and 'delete' actions
                'actions'=>array('admin','delete','deleteEselonEmpat'),
                'users'=>array('@'),
	...
	{
		$model=new Indikatorkegiatan();

		// Uncomment the following line if AJAX validation is needed
		// $this->performAjaxValidation($model);

		if (isset($_POST['Indikatorkegiatan'])) {
			$model->attributes=$_POST['Indikatorkegiatan'];
			if ($model->save()) {
	...
	{
		$model=new IndikatorKegiatanEselonEmpat();

		// Uncomment the following line if AJAX validation is needed
		// $this->performAjaxValidation($model);

		if (isset($_POST['IndikatorKegiatanEselonEmpat'])) {
			$model->attributes=$_POST['IndikatorKegiatanEselonEmpat'];
			if ($model->save()) {
	...
	public function actionDeleteEselonEmpat($id)
	{
		if (Yii::app()->request->isPostRequest) {
			// we only allow deletion via POST request
            $model = $this->loadModelEselonEmpat($id);
            $dataanak = KegiatanEselonEmpat::model()->findAllByAttributes(
                array('nomor_misi'=>$model->nomor_misi,
                    'id_instansi'=>$model->id_instansi,
                    'nomor_tujuan'=>$model->nomor_tujuan,
                    'nomor_sasaran'=>$model->nomor_sasaran,
	...
	{
        $model=new Indikatorkegiatan('search');
        $model->unsetAttributes();  // clear any default values
        if (isset($_GET['Indikatorkegiatan'])) {
        $model->attributes=$_GET['Indikatorkegiatan'];
        }
	...
	{
        $model=new IndikatorKegiatanEselonEmpat('search');
        $model->unsetAttributes();  // clear any default values
        if (isset($_GET['IndikatorKegiatanEselonEmpat'])) {
        $model->attributes=$_GET['IndikatorKegiatanEselonEmpat'];
        }
	 ...
	 * @return Indikatortujuan the loaded model
	 * @throws CHttpException
	 */
	public function loadModel($id)
	{
		$model=Indikatorkegiatan::model()->findByPk($id);
		if ($model===null) {
	...
	{
		$model=IndikatorKegiatanEselonEmpat::model()->findByPk($id);
		if ($model===null) {
			throw new CHttpException(404,'The requested page does not exist.');
		}
		return $model;
	...
	{
		if (isset($_POST['ajax']) && $_POST['ajax']==='Indikatorkegiatan-form') {
