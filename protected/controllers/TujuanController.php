...
class TujuanController extends Controller
{
	...
	public function actionCreate()
	{
		...
		if (isset($_POST['Tujuan'])) {
			$model->attributes=$_POST['Tujuan'];
			if ($model->save()) {
				$this->redirect(array('view','id'=>$model->tujuanid));
			}
		}
	...
	{
        $model=new Tujuan('search');
        $model->unsetAttributes();  // clear any default values
        if (isset($_GET['Tujuan'])) {
            $model->attributes=$_GET['Tujuan'];
        }
	...
	{
		$model=Tujuan::model()->findByPk($id);
		if ($model===null) {
	 ...
	 * @param Tujuan $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if (isset($_POST['ajax']) && $_POST['ajax']==='tujuan-form') {
